...
import { beverages, souces, syrups, buildCheckBeverageMessage } from "../helper/Beverages";
...
const Home = () => {
                    ...
                    >
                        {disabled && (
                            <Box sx={{
                                ...
                                <Typography fontSize={10} color="#c0392b" fontWeight={700}>Stok yok</Typography>
                            </Box>
                        )}
                        ...
                        <Box sx={{ display: "flex", gap: 0.75 }}>
                            ...
                            <Box sx={{
                                ...
                                <Typography fontSize={9} color="#a08060" fontWeight={500}>
                                    {item.milkChocolate ? "Kahvesiz" : item.caffeine ?? "—"}
                                </Typography>
                            </Box>
                        </Box>
    ...
    if (machineLoading) return fullscreen("Makine durumu kontrol ediliyor...");
    if (machineErr) return fullscreen(`Bağlantı hatası: ${machineErr}`, "#F57C00");
    if (!machine.online) return fullscreen("Kahve makinesi çevrimdışı.", "#c0392b");
    if (machine.has_blocking_error) return fullscreen("Kahve makinesi kullanılamıyor.", "#c0392b");
    if (machine.cleaning) return fullscreen("Kahve makinesi temizlik modunda.");


    // ───────────────────── RENDER ─────────────────────
    return (
        <Box sx={{ minHeight: "100vh", backgroundColor: "#2e2720", display: "flex", flexDirection: "column" }}>
            ...
            {/* ── Stok Bar ── */}
            <StockBar stockStatus={stockStatus} />

            {/* ── Uyarı bantları ── */}
            {(!robotStatus.can_order && !robotLoading) && (
                <Box sx={{
                    mx: { xs: 2, md: 4 }, mt: 1.5, p: 1.25, borderRadius: "8px",
                    backgroundColor: "rgba(192,57,43,0.15)", border: "1px solid rgba(192,57,43,0.4)",
                }}>
                    <Typography fontSize={12} fontWeight={600} color="#c0392b">
                        {!robotStatus.connected ? "⚠ Robot bağlantısı yok — sipariş alınamıyor"
                            : robotStatus.mode_label === "manual" ? "⚠ Robot manuel modda — otomatik moda alın"
                                : `⚠ Robot hazır değil (${robotStatus.state_label})`}
                    </Typography>
                </Box>
            )}
            {milkDisabled && !allDisabled && (
                <Box sx={{
                    ...
                    <Typography fontSize={12} fontWeight={600} color="#F57C00">
                        ⚠ Süt miktarı düşük — sütlü içecekler devre dışı
                    </Typography>
                </Box>
            )}
            {chocDisabled && !allDisabled && (
                <Box sx={{
                    ...
                    <Typography fontSize={12} fontWeight={600} color="#F57C00">
                        ⚠ Çikolata miktarı düşük — çikolatalı içecekler devre dışı
                    </Typography>
                </Box>
            )}
            ...
            <Box sx={{
                    ...
                    .map((categories) => (
                        ...
                        >
                            <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                                ...
                                />
                                <Typography fontSize="26px" color="#E7D9CF">{categories.name}</Typography>
                            </Box>
                        </Box>
                    ))
                )}
                ...
                {chocolate_page && (() => {
                    ...
                    return renderPage(iceList, "Ice Cream List");
                })()}
            </Box>
            ...
            >
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2, p: 1 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        ...
                        <Typography fontSize={20} fontWeight="bold" color="#DEC39B">Sepet</Typography>
                    </Box>
                            ...
                            >
                                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                    ...
                                    <Box sx={{ flex: 1, px: 1.5 }}>
                                        <Typography fontSize={15} fontWeight="bold" color="#E7D9CF">{item.Name}</Typography>
                                ...
                                {item.customize && item.customize.length > 0 && (
                                    <Box sx={{ mt: 1, pl: 1, borderTop: "1px solid rgba(222,195,155,0.1)", pt: 1 }}>
                                        <Typography fontSize={12} color="#a08060" mb={0.5}>İlaveler:</Typography>
                                        {item.customize.map((name) => {
                                            ...
                                            return (
                                                <Box key={name} sx={{
                                                    ...
                                                    <Typography fontSize={13} color="#E7D9CF">+ {name}</Typography>
                ...
                <Box sx={{ pt: 2, borderTop: "1px solid rgba(222,195,155,0.3)" }}>
                    {!canOrder && Object.keys(cart).length > 0 && (
                        <Box sx={{
                            ...
                            <Typography fontSize={11} color="#c0392b">
                                {allDisabled ? "Stok yetersiz."
                                    : !robotStatus.can_order
                                        ? robotStatus.mode_label === "manual" ? "Robot manuel modda." : "Robot hazır değil."
                                        : "Makine hazır değil."}
                            </Typography>
                        </Box>
                    )}
                    ...
                    >
                        Sipariş Ver · {totalCartItems} içecek
                    </Button>
                </Box>
            </Drawer>
